...
async function run_with_user_input(
	editor: Editor,
	view: MarkdownView,
	...
	if (!opt_spawner_file) {
		new Notice(
			`Error: Please use this command from within a note to spawn from`
		);
		return;
	}
	const spawner_file = opt_spawner_file;

	const spawner_is_index = notecluster.is_core_file(spawner_file);

	// We only require it be a small note file if it's not spawned from outside or the index
	if (
		!spawner_is_index &&
		!opt_selected_bignote_root_folder &&
		!notecluster.is_peripheral_file(view, spawner_file)
	) {
	...
	const core_file = opt_index_file;

	const opt_bignote_root_folder = core_file.parent;
	...
	const opt_context_folder_name = notecluster.context_type_singular_convert(
		selected_context,
		notecluster.CONTEXT_TYPE_FOLDERS
	);
	...
	const opt_context_block_identifier_code =
		notecluster.context_type_singular_convert(
			selected_context,
			notecluster.CONTEXT_TYPE_BLOCK_IDENTIFIER_CODE
		);
	...
	const context_folder_path = normalizePath(
		path.join(bignote_root_folder.path, context_folder_name)
	);
	...
	const res_triplet_id = notecluster.get_next_triplet_id_for_folder(
		context_folder_name,
		core_file
	);
	if (res_triplet_id instanceof Error) {
		new Notice(`Error: Failed to generate new triplet id`);
		return;
	}
	const triplet_id = res_triplet_id;

	const new_note_basename = `${triplet_id} ${new_note_name}`;

	const block_identifier = `^spawn-${context_block_identifier_code}-${id6}`;

	comm.insert_text_at_editor_cursor_in_own_line(
		editor,
		`Spawn [[${new_note_basename}]] ${block_identifier}`
	);

	const new_file_path = normalizePath(
		path.join(context_folder_path, new_note_basename + ".md")
	);
	...
	if (notecluster.context_type_heading_singular_is_doer(selected_context)) {
...
export class SelectContextTypeModal extends FuzzySuggestModal<string> {
	...
	constructor(
		...
		editor: Editor,
		view: MarkdownView,
		opt_selected_index_folder: TFolder | undefined
	) {
		super(app);
		this.editor = editor;
		this.view = view;
	...
	getItems(): string[] {
		return notecluster.CONTEXT_TYPE_HEADINGS_SINGULAR;
	}
	...
	onChooseItem(item: string, evt: MouseEvent | KeyboardEvent) {
		new InputNoteNameModal(this.app, (result) => {
			run_with_user_input(
				this.editor,
				this.view,
				this.opt_selected_bignote_root_folder,
				item,
				result
			);
		}).open();
...
export class SelectBignoteRootFolderModal extends FuzzySuggestModal<TFolder> {
	...
	getItems(): TFolder[] {
		return notecluster
			.get_all_cluster_folders_in_vault(this.view)
	...
	onChooseItem(item: TFolder, evt: MouseEvent | KeyboardEvent) {
		new SelectContextTypeModal(
			...
			this.editor,
			this.view,
			/*opt_selected_bignote_root_folder*/ item
		).open();
	}
}
...
export async function run(editor: Editor, view: MarkdownView) {
	new SelectContextTypeModal(
		...
		editor,
		view,
		/*opt_selected_bignote_root_folder*/ undefined
	).open();
}
